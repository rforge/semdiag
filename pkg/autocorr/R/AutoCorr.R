...
autocorr.test <- function(x,lagmax=3,
	...
	if(sum(is.na(x))>0){
		stop('Error: Data Contain Missing Values')
	}
	x = as.matrix(x)
	nP = ncol(x)
	nT = nrow(x)
	...
	acf.est<-acf(x,lag.max = lagmax,plot=F)$acf[2:(lagmax+1)]
	
	if (length(grep(method,"asymptotic"))>0){
		se = sqrt(1/nT)
		z.stat = abs(acf.est/sqrt(1/nT))
		sig		<- (z.stat > zcrit)
		res = cbind(acf.est,rep(se,3),z.stat,sig)
		...
		ahat	<- try(JN.VMB(x,lagmax,blocksize))
...
BartlettSE <- function(x,L,lagmax){
	T = length(x)
	rho_hat = acf(x,lag.max=L+lagmax,type="correlation",plot=F)$acf[1:(L+lagmax+1)]
...
##-----------------------------------------------##
JN.NB<-function(x,lagmax){
	n.t<-length(x)
	ahat<-matrix(NA,ncol=lagmax,nrow=1)
	acf.y<-matrix(NA,nrow=n.t,ncol=lagmax)
	for(i in 1:n.t){
		data.j<-x[-i]
		acf.y[i,]<-acf(data.j,lag.max=lagmax,type="correlation",plot=F)$acf[2:(lagmax+1)]
	}
	for(i in 1:lagmax){
		temp=mean(acf.y[,i])-acf.y[,i]
		temp2=t(temp)%*%(temp^2)
...
##  Surrogate
##----------------------------------------------##
Surrogate<-function(acf.est,ahat,x,a1,a2,boot,lagmax){
	acf.y<-matrix(NA,nrow=boot,ncol=lagmax)
	n.t<-length(x)
	for(i in 1:boot){
		data.b<-x[sample(1:n.t,n.t,replace=FALSE)]
		acf.y[i,]<-acf(data.b,lag.max=lagmax,type="correlation",plot=F)$acf[2:(lagmax+1)]
	}
	se.acf<-apply(acf.y,2,sd)
...
VMB<-function(acf.est,ahat,x,l,a1,a2,B,lagmax){
	acf.y<-matrix(NA,nrow=B,ncol=lagmax)
	n.t<-length(x)
	k <- floor(n.t/l)
	...
	for(i in 1:B){
		...
		acf.y[i,]	<-	corr.VMB(pair.id.M,x)
	}
	se.acf	<-apply(acf.y,2,sd, na.rm=TRUE)
	CI.per	<-Perc.CI(acf.y,a1,a2)
	CI.BCa	<-BCa.CI(acf.y,acf.est,B,ahat,a1,a2)
	return(list(se = se.acf,CI = list(per = CI.per,BCa = CI.BCa)))
}
...
BCa.CI	<- function(est.b,est,B,ahat,a1,a2){
	...
	est.b<-apply(est.b,2,sort, na.last = TRUE)
	num	<- apply(rbind(est,est.b),2,function(x) sum(x[2:(B+1)] < x[1], na.rm=TRUE))
	z0	<- qnorm(num/B)
	if(max(num) == B){z0[which(num==B)]=1000}
