...
plot.webpower <- function(x, xvar = NULL, yvar = NULL, xlab = NULL, ylab = NULL, 
    ...
    if (check == 2) {
        ...
        plot(x$n, x$power, xlab = xlab, ylab = ylab, ...)
        lines(x$n, x$power, ...)
    } else if (check == 1) {
        ...
        plot(x.matrix[, xvar], x.matrix[, yvar], xlab = xlab, ylab = ylab, 
            ...)
        lines(x.matrix[, xvar], x.matrix[, yvar], ...)
    }
}

# Sove An Single Equation
...
#########################################################
## Statistical Power Analysis for ANOVA with Binary or Count ##
...
########################################################
## Statistical Power Analysis for Multiple way ANOVA analysis ##
wp.kanova<-function (n = NULL, ndf = NULL, f = NULL, ng=NULL, sig.level = 0.05, power = NULL) 
{
    if (sum(sapply(list(n, ndf, f, ng, power, sig.level), is.null)) != 
        1) 
        stop("exactly one of u, v, f, power, and sig.level must be NULL")
    if (!is.null(f) && f < 0) 
        stop("f must be positive")
    if (!is.null(n) && n < 1) 
    ...
    p.body <- quote({
        lambda <- f^2 * n
		ddf <- n - ng
        pf(qf(sig.level, ndf, ddf, lower = FALSE), ndf, ddf, lambda, 
            lower = FALSE)
    })
    if (is.null(power)) 
        power <- eval(p.body)
    ...
    structure(list(n = n, ndf = ndf, ddf=ddf, f = f,  ng=ng, sig.level = sig.level, 
...
############################################################################## CRT
## Statistical Power Analysis for Cluster Randomized Trials ##
...
#######################################################
#### Effect size calculatation for CRT #### 
estCRT2arm <- function(dat) {
    #require(lme4)
...
wp.effect.CRT2arm <- function(dat) {
    #dat <- read.table(file, header = TRUE)
    ...
    METHOD <- "Effect size for CRT2arm"
    URL <- "http://psychstat.org/crt2arm"
    structure(list(f = f, ICC = rho, 
        note = NOTE, method = METHOD, url = URL), class = "webpower")
}


# Effect size calculatator based on raw data for Cluster Randomized Trials with 3 Arms
wp.effect.CRT3arm <- function(dat) {
    #dat <- read.table(file, header = TRUE)
    ...
    METHOD <- "Effect size for CRT3arm"
    URL <- "http://psychstat.org/crt3arm"
    structure(list(f1 = f1, f2 = f2, f3 = f3, ICC = rho, 
                   note = NOTE, method = METHOD, url = URL), class = "webpower")
}
#
############################################################################## MRT
## Statistical Power Analysis for Multisite Randomized Trials ##
#############################
...
wp.effect.MRT2arm <- function(dat) {
    #dat <- read.table(file, header = TRUE)
    ...
    #return(list(f = f))
    NOTE <- "f is the effect size."
    METHOD <- "Effect size for MRT2arm"
    URL <- "http://psychstat.org/mrt2arm"
    structure(list(f = f, 
                   note = NOTE, method = METHOD, url = URL), class = "webpower")
}

## Effect size for MRT with 3 arms based on empirical data

# Effect size calculatator based on raw data for Multisite Randomized Trials with 3 Arms
wp.effect.MRT3arm <- function(dat) {
    #dat <- read.table(file, header = TRUE)
    ...
    METHOD <- "Effect size for MRT3arm"
    URL <- "http://psychstat.org/mrt3arm"
    structure(list(f1 = f1, f2 = f2, 
                   note = NOTE, method = METHOD, url = URL), class = "webpower")
}
...
############################################################################## SEM
## Statistical Power Analysis for Structural Equation Modeling ##
...
summary.power<-function(object, ...) {
    ...
    for(g in 1:object@Data@ngroups) {
        ...
        # 4. intercepts/means
        ord.names <- lavNames(object@ParTable, type="ov.ord", group=g)
...
wp.mc.sem.boot<-function(model, indirect=NULL, nobs=100, nrep=1000, nboot=1000, alpha=.95, skewness=NULL, 
		kurtosis=NULL, ovnames=NULL, se="default", estimator="default", parallel="no",
...
############################# power curve for SEM based on Monte Carlo
